...
#include <stdint.h>    // Para los tipos estándar (uint8_t, etc.)
#include "stm32l4xx_hal.h"  // Para la HAL de STM32L4
#include "main.h"
...
int main(void)
{
  ...
  while (1)
  {
    ...
    if (HAL_GetTick() - last_heartbeat_time >= 1000) // Cada 500 ms  // Cambia el estado del LED LD2 1000 → parpadeo más lento (1 vez por segundo).
    {
    ...
    if (button_flag == 1)
    {
      ...
      HAL_GPIO_WritePin(LD2_GPIO_Port, LD2_Pin, GPIO_PIN_SET); // Enciende LD2 también


      // 2. Calcular cuándo apagarlo (3 segundos desde ahora) 
      led_ext_off_time = HAL_GetTick() + 10000;  //Aqui se puede cambiar el tiempo que el LED externo dure encendido. Tempo en milisegundos.

      // 3. Enviar mensaje por UART
      sprintf(tx_buffer, "Boton B1 detectado. Acceso Concedido!\r\n");
    ...
    if (led_ext_off_time != 0 && HAL_GetTick() >= led_ext_off_time)
    {
      ...
      HAL_GPIO_WritePin(LD2_GPIO_Port, LD2_Pin, GPIO_PIN_RESET); // Apaga LD2 también
